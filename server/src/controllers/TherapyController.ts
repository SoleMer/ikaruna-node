...
class TherapyController {
  ...
  async CreateTherapy(req: Request, res: Response) {
    const { therapy } = req.body;
      ...
      try {
        ...
        if (therapy) {
          return res.status(201).json({ message: postResponseMessages.therapy_created_successfully });
        } else {
          return res.status(401).json({ message: postResponseMessages.incomplete_data });
        }
    } catch (error) {
      return res.status(501).json({ message: "Internal server error" });
    }
  ...
  async DeleteTherapy(req: Request, res: Response) {
    ...
    try {
        ...
        return res.status(401).json({ message: delResponseMessages.therapy_deleted_successfully });
    } catch (error) {
  ...
  async updateTherapy(req: Request, res: Response) {
    ...
    try {
      ...
      return res.status(201).json({ message: putResponseMessages.updated_therapy });
    } catch (error) {
      return res.status(501).json({ message: "Internal server error" });
    }
  }
